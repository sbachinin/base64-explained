...
Such statements can lead you to think that something binary becomes non-binary. (Because there is this strange convention that textual data is something opposite to binary data. I complained about it in detail [here](https://goblog-production.up.railway.app/articles/dev/the-notion-of-binary-file-is-confusing-you)). In fact all ASCII characters produced by base64 are ultimately bits and bytes, just like the original data.
